...
dtau_u = min(1.0./(7/3*(Kv(:,:,imx)+Kv(:,:,ipx))./2./(h/2)^2 + (1-omvx).*(Cv(:,:,imx)+Cv(:,:,ipx))/2), [], 1).*ones(NPHS,1);  % [Pas/m2]
dtau_w = min(1.0./(7/3*(Kv(:,imz,:)+Kv(:,ipz,:))./2./(h/2)^2 + (1-omvz).*(Cv(:,imz,:)+Cv(:,ipz,:))/2), [], 1).*ones(NPHS,1);  % [Pas/m2]
dtau_p = min(1.0./(  4* Kf                         ./(h/2)^2 + (1-omfc).* Cf                        ), [], 1).*ones(NPHS,1);  % [1/Pas]
...
% dtau_u =         sqrt(ndim)/Re./( (Kv(:,:,imx)+Kv(:,:,ipx))./2./(h/2)^2 + (1-omvx).*(Cv(:,:,imx)+Cv(:,:,ipx))/2 );
% dtau_w =         sqrt(ndim)/Re./( (Kv(:,imz,:)+Kv(:,ipz,:))./2./(h/2)^2 + (1-omvz).*(Cv(:,imz,:)+Cv(:,ipz,:))/2 );
% dtau_p = r/(r+2)^2/sqrt(ndim)./(           Kf                ./(h/2)^2 + (1-omfc).* Cf                         );
