...
fNormErr = norm(fTrue(:)-f(:),2)./norm(fTrue(:),2);
pNormErr = norm(pTrue(:)-p(:),2)./norm(pTrue(:),2);
uNormErr = norm(uTrue(:)-u(:),2)./norm(uTrue(:),2);
wNormErr = norm(wTrue(:)-w(:),2)./norm(wTrue(:),2);

fprintf(1, '\n\n');
fprintf(1,'norm [f,p,u,w] error = %.6e, %.6f, %.6f, %.6f.\n', ...
...
plot_field((u-uTrue)./(uTrue+1e-16), 'normalized u error');
plot_field(wTrue, 'wTrue'); plot_field(w, 'wNum');
plot_field(pTrue, 'pTrue'); plot_field(p, 'pNum');
plot_field((pTrue-p)./pTrue, 'normalized p error');
