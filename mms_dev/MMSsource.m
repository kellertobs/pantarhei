...
function  [usrc, wsrc, psrc, fsrc] = main (time, Din, Nin, Params, MfSol)
...
u        = Calc_u(time, XuGrid, ZuGrid, Tu, Xu, Zu, U0, dU0);
w        = Calc_w(time, XwGrid, ZwGrid, Tw, Xw, Zw, W0, dW0);
p        = Calc_p(time, X     , Z     , Tp, Xp, Zp, P0, dP0);
[f,dfdt] = Calc_f(time, X     , Z     , Tf, Xf, Zf, f0, df0);
...
usrcFunc  = @(t) Calc_usrc(t,XuGrid(1,:,:),ZuGrid(1,:,:),...
    d0,eta0,rho0,rhomix,grav,A,B,C,Gm,...
    Tp,Xp,Zp,P0,dP0, Tf,Xf,Zf,f0,df0, Tu,Xu,Zu,U0,dU0, Tw,Xw,Zw,W0,dW0,...
    thtlim, cfflim);

wsrcFunc  = @(t) Calc_wsrc(t,XwGrid(1,:,:),ZwGrid(1,:,:),...
    d0,eta0,rho0,rhomix,grav,A,B,C,Gm,...
    Tp,Xp,Zp,P0,dP0, Tf,Xf,Zf,f0,df0, Tu,Xu,Zu,U0,dU0, Tw,Xw,Zw,W0,dW0,...
    thtlim, cfflim);

pfsrcFunc = @(t) Calc_pfsrc(t,X(1,:,:),Z(1,:,:),...
    d0,eta0,rho0,rhomix,grav,A,B,C,Gm,...
    Tp,Xp,Zp,P0,dP0, Tf,Xf,Zf,f0,df0, Tu,Xu,Zu,U0,dU0, Tw,Xw,Zw,W0,dW0,...
    thtlim, cfflim);
